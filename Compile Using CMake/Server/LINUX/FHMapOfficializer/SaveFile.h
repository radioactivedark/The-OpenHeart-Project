...
const std::vector<String> stringSplit(const String &szLine, const String &szDelim, const int &nMax)
{
    std::vector<String> vszTokens;
    ...
    for(int i=0; i<(int)szLine.length(); i++)
    {
        ...
        if(nNumSplits < nMax && szDelim.find(c) != std::string::npos)
        {
            if(szBuffer != "")
            {
                vszTokens.push_back(szBuffer);
            }
            szBuffer = "";
            nNumSplits++;
        }
...
public:
    ...
    const bool load(const String &filename)
    {
        ...
        while(inFile.good())
        {
            ...
            else
            {
                ...
                const std::vector<String> part = stringSplit(nextLine,"=",1);
                //No tag, just value
    ...
    const bool loadZip(const String &filename, const String &zipFilename)
    {
        ...
        ifstream infile(zipFilename.c_str());
        ...
        while(!inStream->eof())
        {
            ...
            else
            {
                ...
                const vector<String> part = StringUtil::split(nextLine,"=",1);
                //No tag, just value
                if(part.size()<2)setting->mValue = nextLine;
                //Both tag and value
                else
    ...
    */
    ...
    /*
    static const vector<String> findResourceNames(const String &group, const String &wildcard)
    {
    ...
    static const vector<String> findResourceNames(const String &wildcard)
    {
