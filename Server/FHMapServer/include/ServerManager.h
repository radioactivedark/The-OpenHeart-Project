...
public:
    ...
    void connectEvent(const SystemAddress &add, const string &token, const bool &firstEntrance, const bool &ghostMode=false)
    {
        ...
        for(TokenMap::iterator it=mToken.begin(); it!=mToken.end(); it++)
        {
            ...
            if(target!=add)
            {
                if(!targetToken->mGhostMode)mNetMgr->sendCharInfo(add,targetToken->mCharID,&targetToken->mCharData,targetToken->mPosX,targetToken->mPosY,targetToken->mPosZ,targetToken->mUserID,targetToken->mUsername);
            ...
            case CHAT_WHISPER:
                {
                    ...
                    if(userID==0 || userID==senderID)mNetMgr->sendChat(add,0,"",0,channel,whisperTarget,true);
                    else
                    {
                        if(!mDBMgr->isUserOnline(userID))
                        {
                            mNetMgr->sendChat(add,0,"",0,channel,whisperTarget,true,true);
                            return;
                        }
                        ...
                        if(target.second)
                        {
                            mNetMgr->sendChat(target.first,charID,caption,channel,senderID,sender->second.mUsername);
                        }
                        //Player is in another map
                        else mNetMgr->relayChat(userID,charID,caption,channel);
                    }
                }
                break;
            default:
            ...
            case CHAT_WHISPER:
                ...
                }*/
                {
                    ...
                    if(target.second)
                    {
                        ...
                        mNetMgr->sendChat(target.first,charID,caption,channel,sayerID,whisperer);
                    }
                }
                break;
            case CHAT_SYSTEM:
    ...
    void charEmote(const SystemAddress &add, const unsigned char &emote)
    {
        TokenMap::iterator sender = mToken.find(add);
        if(sender==mToken.end())return;
    ...
    void modEvent(const SystemAddress &add, const string &command)
    {
        ...
        if(cmd==COMMAND_HELP)
        {
            ...
            if(mDBMgr->getModPrivilege(token->second.mUserID,"kick"))info += "/kick [username];";
            if(mDBMgr->getModPrivilege(token->second.mUserID,"ban"))info += "/ban [username] [value] [min/hour/day/week/month/year];";
        ...
        else if(cmd==COMMAND_KICK)
        {
            ...
            if(target.second)
            {
                mNetMgr->sendMod(target.first,command);
                mNetMgr->sendMod(add,command);
            }
        ...
        else if(cmd==COMMAND_BAN)
        {
            ...
            if(privilege==0)return;
            const vector<string> part = tokenize(content);
            if(part.size()>=3)
            {
                //Spaced usernames
                ...
                if(targetID)
                {
                    ...
                    const pair<SystemAddress,PlayerToken*> target = getPlayerTokenByUserID(targetID);
                    if(target.second)mNetMgr->sendMod(target.first,command2);
                    mNetMgr->sendMod(add,command2);
                }
    ...
    const vector<string> tokenize(string caption)
    {
        vector<string> result;
        while(caption.length()>0)
        {
            const size_t i = caption.find_first_of(' ');
            if(i==string::npos)
            {
    ...
    void myGroupLeaveEvent(const SystemAddress &add, unsigned int charID, const bool &isKick)
    {
        ...
        if(isDisband)
        {
            mDBMgr->deleteAllGroupInvite(groupID,false);
            mDBMgr->deleteAllGroupRequest(groupID,true);
