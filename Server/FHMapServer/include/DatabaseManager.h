...
public:
    ...
    {
        string queryBuffer = "SELECT name, pos_x, pos_y, pos_z, group_id, party_id FROM fh_characters WHERE id="+toString(id);
        ...
        queryBuffer += "FROM fh_appearance WHERE id="+toString(id);
    ...
    {
        string queryBuffer = "UPDATE fh_characters SET ";
    ...
    {
        string queryBuffer = "SELECT id FROM jos_users WHERE username='"+username+"' limit 1";
    ...
    {
        Query query = mDB.query("SELECT username FROM jos_users WHERE id="+toString(id));
        StoreQueryResult res = query.store();
        if(res && !res.empty())
        {
            Row row = res.front();
    ...
    {
        Query query = mDB.query("SELECT name FROM fh_characters WHERE id="+toString(id));
        StoreQueryResult res = query.store();
        if(res && !res.empty())
        {
            Row row = res.front();
    ...
    {
        Query query = mDB.query("SELECT user_id FROM fh_characters WHERE id="+toString(id));
        StoreQueryResult res = query.store();
        if(res && !res.empty())
        {
            Row row = res.front();
    ...
    const string getUserNameByCharID(const unsigned int &id, unsigned int *userID=0)
    {
        ...
        Query query = mDB.query("SELECT username FROM jos_users WHERE id="+toString(nUserID));
        StoreQueryResult res = query.store();
        if(res && !res.empty())
        {
            Row row = res.front();
    ...
    {
        Query query = mDB.query("INSERT INTO fh_friend VALUES("+toString(senderID)+","+toString(targetID)+")");
        SimpleResult res = query.execute();
        if(!res)return false;
        return true;
    }
    ...
    {
        Query query = mDB.query("INSERT INTO fh_block VALUES("+toString(senderID)+","+toString(targetID)+")");
        SimpleResult res = query.execute();
        if(!res)return false;
        return true;
    }
    ...
    {
        Query query = mDB.query("INSERT INTO fh_friend_request VALUES("+toString(senderID)+","+toString(targetID)+")");
        SimpleResult res = query.execute();
        if(!res)return false;
        return true;
    }
    ...
    {
        Query query = mDB.query("SELECT is_online FROM fh_characters WHERE id="+toString(charID));
        StoreQueryResult res = query.store();
        if(res && !res.empty())
        {
            Row row = res.front();
    ...
    {
        Query query = mDB.query("SELECT is_online FROM fh_characters WHERE user_id="+toString(userID));
        StoreQueryResult res = query.store();
        if(res)
        {
    ...
    {
        string queryBuffer = "SELECT map FROM fh_characters WHERE ";
    ...
    {
        Query query = mDB.query("SELECT bio FROM fh_characters WHERE id="+toString(charID));
        StoreQueryResult res = query.store();
        if(res && !res.empty())
        {
            Row row = res.front();
    ...
    {
        Query query = mDB.query("SELECT group_id FROM fh_characters WHERE id="+toString(id));
        StoreQueryResult res = query.store();
        if(res && !res.empty())
        {
            Row row = res.front();
    ...
    {
        Query query = mDB.query("UPDATE fh_characters SET group_id="+toString(groupID)+",group_title=DEFAULT,group_rank="+toString(int(rank))+" WHERE id="+toString(charID));
        SimpleResult res = query.execute();
    ...
    {
        Query query = mDB.query("UPDATE fh_characters SET group_id=0 WHERE group_id="+toString(groupID));
        query.execute();
    ...
    const vector<GroupMemberInfo> getGroupMemberList(const unsigned int &groupID)
    {
        ...
        Query query = mDB.query("SELECT id,name,group_rank,group_title,is_online FROM fh_characters WHERE group_id="+toString(groupID));
        StoreQueryResult res = query.store();
        if(res)
        {
            for(StoreQueryResult::iterator it=res.begin(); it!=res.end(); it++)
    ...
    const vector<pair<unsigned int,string> > getGroupList(string filter)
    {
        ...
        if(res)
        {
            for(StoreQueryResult::iterator it=res.begin(); it!=res.end(); it++)
            {
                ...
                groupList.push_back(pair<unsigned int,String>(groupID,groupname));
            }
        }
        return groupList;
    }
    ...
    {
        Query query = mDB.query("INSERT INTO fh_group_request VALUES("+toString(charID)+","+toString(groupID)+")");
        SimpleResult res = query.execute();
        if(!res)return false;
        return true;
    }
    ...
    {
        Query query = mDB.query("INSERT INTO fh_group_invite VALUES("+toString(groupID)+","+toString(charID)+")");
        SimpleResult res = query.execute();
        if(!res)return false;
        return true;
    }
    ...
    {
        Query query = mDB.query("SELECT home_map,home_x,home_y,home_z FROM fh_characters WHERE id="+toString(charID));
        StoreQueryResult res = query.store();
        if(res && !res.empty())
        {
            Row row = res.front();
    ...
    const vector<PartyMemberInfo> getPartyMemberList(const unsigned int &partyID)
    {
        ...
        Query query = mDB.query("SELECT id,name FROM fh_characters WHERE is_online=1 AND party_id="+toString(partyID));
        StoreQueryResult res = query.store();
        if(res)
        {
            for(StoreQueryResult::iterator it=res.begin(); it!=res.end(); it++)
    ...
    {
        Query query = mDB.query("SELECT party_id FROM fh_characters WHERE is_online=1 AND id="+toString(id));
        StoreQueryResult res = query.store();
        if(res && !res.empty())
        {
            Row row = res.front();
    ...
    {
        Query query = mDB.query("UPDATE fh_characters SET party_id="+toString(partyID)+" WHERE id="+toString(charID));
        SimpleResult res = query.execute();
