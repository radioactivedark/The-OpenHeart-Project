...
public:
    ...
    virtual void charEmote(const SystemAddress &add, const unsigned char &emote) = 0;
...
public:
    ...
    void sendCharInfo(const SystemAddress &add, const unsigned int &charID, CharacterData *data, const float &x, const float &y, const float &z, const unsigned int &userID, const string &username)
    {
        ...
        for(int i=0;i<5;i++)mWriter.writeUChar(data->mMesh[i]);
        for(int i=0;i<4;i++)mWriter.writeUChar(data->mMat[i]);
        for(int i=0;i<10;i++)mWriter.writeChar(data->mScale[i]);
        for(int i=0;i<9;i++)
    ...
    void sendChat(const SystemAddress &add, const unsigned int &charID, const string &caption, const unsigned char &channel, const unsigned int &userID=0, const string &whisperer="", const bool &fail=false, const bool &isOffline=false)
    {
        ...
        if(userID!=0)mWriter.writeUInt(userID);
        if(whisperer!="")
        {
    ...
    {
        mReader.load(p);
        mListener->charEmote(p->systemAddress,mReader.readUChar());
    }
    ...
    void sendMod(const SystemAddress &add, const string &command)
    {
